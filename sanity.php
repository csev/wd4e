...
/**
 * Installation sanity checks for the WD4E home page only.
 *
 * Verifies that Tsugi is present, config.php exists, and the database is usable
 * before top.php starts a session and nav.php builds the site menu.
 */

function wd4e_sanity_die($title, $body_html) {
    error_log('WD4E sanity: '.$title);
...
if ( ! is_dir($tsugi_dir) ) {
    wd4e_sanity_die(
        ...
        '<p>This copy of Web Design for Everybody expects a <code>tsugi</code> folder next to
        <code>index.php</code>, but that folder was not found.</p>
        <p>From your <code>wd4e</code> directory, check out Tsugi:</p>
        <pre>cd wd4e
git clone https://github.com/tsugiproject/tsugi.git tsugi</pre>
        ...
        <p>More detail: <a href="https://github.com/alonmelon25/wd4e/blob/master/README.md"
        target="_blank" rel="noopener noreferrer">WD4E README</a></p>'
    );
}
...
if ( count($missing) > 0 ) {
    wd4e_sanity_die(
        ...
        <p>From your <code>wd4e</code> directory, refresh the Tsugi checkout:</p>
        <pre>cd wd4e/tsugi
git pull
...
cp config-dist.php config.php</pre>
        <p>Edit <code>tsugi/config.php</code> for this site. On a production server:</p>
        <pre>$wwwroot      = \'https://www.wd4e.com/tsugi\';   // For Tsugi
...
$CFG->apphome = \'https://www.wd4e.com\';   // For the site</pre>
        <p>Or on localhost:</p>
        <pre>$wwwroot      = \'http://localhost:8888/wd4e/tsugi\';   // For Tsugi
...
$CFG->apphome = \'http://localhost:8888/wd4e\';   // For the site</pre>
        <p>Also set tool folders and lessons for the enclosing site:</p>
        <pre>$CFG->tool_folders = array("admin", "../tools", "../mod");
$CFG->install_folder = $CFG->dirroot."/../mod";
$CFG->context_title = "Web Design for Everybody";
$CFG->lessons = $CFG->dirroot."/../lessons.json";</pre>
